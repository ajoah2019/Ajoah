    ...
    <form class="card-panel" @submit.prevent="verifyOtp">        
      ...
      <div class="row">        
        <div class="field col s12">         
          ...
          <input id="icon_prefix" type="number" class="validate" data-length="20" v-model="phNo">                                     
        </div>             
  ...
  methods: { 
         ...
         ref.get().then(doc => {
           ...
           if(doc.exists){
              ...
              .then(function() {
                  ...
                  M.toast({html: '회원 가입 되었습니다.', classes: 'rounded'})
    ...
    sendOtp(){
        if(this.phNo.length < 10){
          alert('Invalid Phone Number Format !'+this.phNo.length);
        }else{
